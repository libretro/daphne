...
{
	// get/create surface, set pallette colors, createtexturefromsurface, render
	if (g_palette_modified)
	{
		// update color palette for all the surfaces that we are using
		for (int i = 0;; i++)
		{
			// RJS NOTE - since this indicates he want to modify pixels in the FB
			// we should keep these as surfaces for now then UpdateTexture to screen.
