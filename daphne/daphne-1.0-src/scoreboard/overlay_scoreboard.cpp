	...
	if (m_bNeedsRepaint)
	{
		...
		if (m_bVisible)
		{
			...
			if (!m_bThayers)
			{
				...
				// 2017.11.08 - RJS - reorient draw_string("Lives: ", pSurface->w / 6 - 10, 1, pSurface);
				draw_string("Lives: ", pSurface->w / 6 - 19, 1, pSurface);
...
void OverlayScoreboard::update_player_lives (SDL_Surface *pSurface, int player, unsigned int lives)
{
	...
	draw_overlay_leds(&lives, 1,
		player == 0 ? (65 + (5 * OVERLAY_LED_WIDTH)) : pSurface->w - 2 * OVERLAY_LED_WIDTH,
