...
{
	LOGI("In SDL_PrivateJoystickShouldIgnoreEvent, top of routine.");
    if (SDL_joystick_allows_background_events) {
		LOGI("In SDL_PrivateJoystickShouldIgnoreEvent, return false.  Background events allowed.");
		return SDL_FALSE;
    }

    if (SDL_WasInit(SDL_INIT_VIDEO)) {
		// 2017.09.12 - RJS - Since RA is giving us inputs, focus is assumed.  Keep in mind,
		// with out this assumption SDL_KeyboardFocus requires an SDL_Window to work properly which
		// we do not use in the core.
		return SDL_FALSE;
		/*
        if (SDL_GetKeyboardFocus() == NULL) {
            // Video is initialized and we don't have focus, ignore the event.
			LOGI("In SDL_PrivateJoystickShouldIgnoreEvent, return true.  We don't have focus.");
			return SDL_TRUE;
        } else {
			LOGI("In SDL_PrivateJoystickShouldIgnoreEvent, return false.  We do have focus.");
			return SDL_FALSE;
        }
		*/
    }
	...
	LOGI("In SDL_PrivateJoystickShouldIgnoreEvent, bottom of routine, return false.  Fall through.");
	return SDL_FALSE;
}
...
{
	LOGI("In SDL_PrivateJoystickButton, top of routine.  joystick: %d  button: %d  state: %d", (int)joystick, button, state);
    int posted;
#if !SDL_EVENTS_DISABLED
    SDL_Event event;
    ...
    default:
        /* Invalid state -- bail */
		LOGI("In SDL_PrivateJoystickButton, leaving routine.  Invalid state.");
		return (0);
    }
#endif /* !SDL_EVENTS_DISABLED */
    ...
    /* Make sure we're not getting garbage or duplicate events */
    if (button >= joystick->nbuttons) {
		LOGI("In SDL_PrivateJoystickButton, leaving routine.  Invalid button.  button: %d  nbuttons: %d", button, (int) joystick->nbuttons);
		return 0;
    }
	// 2017.09.11 - RJS - We already check for duplicates in retro_run routine.
	/*
    if (state == joystick->buttons[button]) {
		LOGI("In SDL_PrivateJoystickButton, leaving routine.  Duplicate event.  state: %d  buttons[button]: %d", state, joystick->buttons[button]);
		return 0;
    }
	*/
    ...
    if (SDL_PrivateJoystickShouldIgnoreEvent()) {
        if (state == SDL_PRESSED) {
			LOGI("In SDL_PrivateJoystickButton, leaving routine.  Event ignored, SDL_PRESSED.");
			return 0;
        }
    }
    ...
    posted = 0;
#if !SDL_EVENTS_DISABLED
	LOGI("In SDL_PrivateJoystickButton, before create event.");
    if (SDL_GetEventState(event.type) == SDL_ENABLE) {
		LOGI("In SDL_PrivateJoystickButton, creating event.  instance: %d  button: %d  state: %d", (int) joystick->instance_id, button, state);
		event.jbutton.which = joystick->instance_id;
        event.jbutton.button = button;
        event.jbutton.state = state;
        posted = SDL_PushEvent(&event) == 1;
		LOGI("In SDL_PrivateJoystickButton, event posted.  rc: %d", posted);
	}
#endif /* !SDL_EVENTS_DISABLED */
	LOGI("In SDL_PrivateJoystickButton, after create event posted.  rc: %d", posted);
	LOGI("In SDL_PrivateJoystickButton, bottom of routine.");
	return (posted);
}
