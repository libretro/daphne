...
// 2017.07.28 - RJS ADD - Logging and libretro needs. (This block)
...
{
	// 2017.09.20 - RJS - Routine removed, comments left for legacy.
	// RJS - 2017.07.28 - Am I missing something, this damn field doesn't exist anywhere.  We should doc this out.
	// Leaving until I come back from vacation, don't have time to debug if problem.  It's in SDLActivity.java which is
	// no longer in the solution.  If this flag is really important, we need to make another one on the C side of things
	// to check during input from RA.
	// RJS - 2017.08.10 - Rest of routine doc'd out.
	return;
}
...
void Android_InitTouch(void)
{
	...
	LOGI("daphne-libretro: In Android_InitTouch, top of routine.");

	int i;
    int* ids;

	// 2017.09.21 - RJS - Not supporting input device polling on core side
    ...
    if (0 < number) {
        for (i = 0; i < number; ++i) {
			// 2017.02.07 - RJS ADD - Logging.
			LOGI("daphne-libretro: In Android_InitTouch, before SDL_AddTouch.  Num: %d  On: %d  Id: %d", number, i, ids[i]);
            SDL_AddTouch((SDL_TouchID) ids[i], ""); /* no error handling */
			// 2017.02.07 - RJS ADD - Logging.
			LOGI("daphne-libretro: In Android_InitTouch, after SDL_AddTouch.  Num: %d  On: %d  ID: %d", number, i, ids[i]);
        }
        SDL_free(ids);
    }
	// 2017.02.07 - RJS ADD - Logging.
	LOGI("daphne-libretro: In Android_InitTouch, bottom of routine.");
}
