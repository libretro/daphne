		...
		case SDL_KEYDOWN:
			switch(event->key.keysym.sym)
			{
				...
				default:
					...
					if (StringLocation < CHARS_PER_LINE - 1 && event->key.keysym.sym)
					{
						// RJS CHANGE - might need to change for unicode (event SDL_TEXTINPUT)
						// ConsoleLines[0][StringLocation] = event->key.keysym.unicode;
						ConsoleLines[0][StringLocation] = event->key.keysym.sym;
						StringLocation++;
						inputbackground.x = 0;
						inputbackground.y = ConsoleSurface->h-FontHeight(FontNumber);
						inputbackground.w = ConsoleSurface->w;
...
int ConsoleInit(const char *FontName, SDL_Surface *DisplayScreen, int lines)
{
	...
	// RJS CHANGE - AllocSurface was a define CreateRGBSurface which doesn't exist in SDL2
	// Temp = SDL_AllocSurface(SDL_SWSURFACE, OutputScreen->w, OutputScreen->h / 2, OutputScreen->format->BitsPerPixel, 0, 0, 0, 0);
	// If doing surfaces in SDL2, use this: Temp = SDL_CreateRGBSurface(SDL_SWSURFACE, OutputScreen->w, OutputScreen->h / 2, OutputScreen->format->BitsPerPixel, 0, 0, 0, 0);
	Temp = SDL_CreateRGBSurface(SDL_SWSURFACE, OutputScreen->w, OutputScreen->h / 2, OutputScreen->format->BitsPerPixel, 0, 0, 0, 0);
	if(Temp == NULL)
	{
	...
	// RJS CHANGE - DisplayFormat not in SDL2
	// ConsoleSurface = SDL_DisplayFormat(Temp);
	ConsoleSurface = SDL_ConvertSurface(Temp, DisplayScreen->format, 0);
	SDL_FreeSurface(Temp);
	SDL_FillRect(ConsoleSurface, NULL, 0);
	
	/* Load the consoles font */
	// RJS CHANGE - new parm needed for loading the font to a surface
	// if( -1 == (FontNumber = LoadFont(FontName, TRANS_FONT)))
	if( -1 == (FontNumber = LoadFont(FontName, TRANS_FONT, DisplayScreen->format)))
	{
	...
	// If doing surfaces in SDL2, use this: Temp = SDL_CreateRGBSurface(SDL_SWSURFACE, OutputScreen->w, FontHeight(FontNumber), OutputScreen->format->BitsPerPixel, 0, 0, 0, 0);
	Temp = SDL_CreateRGBSurface(SDL_SWSURFACE, OutputScreen->w, FontHeight(FontNumber), OutputScreen->format->BitsPerPixel, 0, 0, 0, 0);
	if(Temp == NULL)
	{
	...
	InputBackground = SDL_ConvertSurface(Temp, DisplayScreen->format, 0);
	SDL_FreeSurface(Temp);
	SDL_FillRect(InputBackground, NULL, 0);
	InputBackground = Temp;
 ...
 * be unloaded */
// RJS CHANGE - made wholesale change to the input "border" to "pcBorderFilename".  Using lower case
// and upper case with the same valiable name is unwise.
int SetConsoleBorder(const char *pcBorderFilename)
{
	SDL_Surface *temp;

	if(pcBorderFilename == NULL && Border != NULL)
	{
	...
	if(NULL == (temp = SDL_LoadBMP(pcBorderFilename)))
	{
		ConOut("Cannot load border %s.", pcBorderFilename);
		return 1;
	}
	if (Border != NULL)
	{
	...
	Border = SDL_ConvertSurface(temp, ConsoleSurface->format, 0);
	SDL_FreeSurface(temp);
	
	// RJS REMOVED - not sure what to do here off hand
	...
	Uint8 nConsoleSurfaceAlpha = 0;
	SDL_GetSurfaceAlphaMod(ConsoleSurface, &nConsoleSurfaceAlpha);
	if (nConsoleSurfaceAlpha)
		SDL_SetSurfaceAlphaMod(Border, nConsoleSurfaceAlpha);
	// RJS END	
...
void ConsoleAlpha( unsigned char alpha )
{
	...
	if(alpha > 0 && alpha <= 200)
	{
		...
		SDL_SetSurfaceAlphaMod(ConsoleSurface, alpha);
		// RJS CHANGE
		// if (Border) SDL_SetAlpha(Border, SDL_SRCALPHA, alpha);
		if (Border) SDL_SetSurfaceAlphaMod(Border, alpha);
	}
	else if(alpha == 0)
	{
		// RJS CHANGE - I think this should just go away, keeping for now.
...
int ConsoleBackground(const char *image, int x, int y)
{
	...
	if(image == NULL && BackgroundImage != NULL)
	{
		...
		// RJS REMOVE - this is unlikely necessary
		// SDL_FillRect(InputBackground, NULL, 0);
		return 0;
	}
